...
describes what users get. Pick a format with `-o`, or let cx choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cx <command> -o table   # aligned columns for reading
cx <command> -o jsonl   # one JSON object per line, for piping
cx <command> -o json    # a single JSON array
cx <command> -o csv     # spreadsheet friendly
cx <command> -o tsv     # tab-separated
cx <command> -o url     # just the URL column
cx <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cx <command> --fields id,title,url
```
...
```bash
cx <command>            # a table, because this is a terminal
cx <command> | wc -l    # JSONL, because this is a pipe
```
